export default {
    ...
    <div class="container mt-4">
        <h2 class="mb-3 text-center">{{ chapterName || "Loading..." }}</h2>
                    ...
                    <div class="col-lg-4 col-md-6 mb-4" v-for="quiz in filteredQuizzes" :key="quiz.id">
                        <div class="card custom-card d-flex flex-column justify-content-between h-100 w-100">
                            <div class="card-body flex-grow-1">
                                <h5 class="card-title">Quiz Date: {{ quiz.date_of_quiz }}</h5>
    ...
    data: function () {
        return {
            ...
            quizzes: [],
            searchQuery: "",
            quiz: { id: null, date_of_quiz: '', start_time: '', time_duration: '', remarks: '' },
            isEdit: false,
            chapterName: ""
        };
    },
    computed: {
        filteredQuizzes: function () {
            ...
            return this.quizzes.filter(function (quiz) {
                return quiz.date_of_quiz.includes(query) || (quiz.remarks && quiz.remarks.toLowerCase().includes(query));
            });
        }
    },
    watch: {
    ...
    methods: {
        fetchQuizzes: async function () {
            ...
            try {
                var res = await fetch(`${location.origin}/api/subjects/${this.subjectId}/chapters/${this.chapterId}/quizzes`, {
                    method: "GET",
                    headers: {
        ...
        openModal: function () {
            this.isEdit = false;
            this.quiz = { id: null, date_of_quiz: '', start_time: '', time_duration: '', remarks: '' };
        ...
        saveQuiz: async function () {
            if (!this.quiz.date_of_quiz || !this.quiz.time_duration || !this.quiz.start_time) {
        ...
        deleteQuiz: async function (quizId) {
            ...
            var token = this.$store.state.auth_token;
            try {
                var res = await fetch(`${location.origin}/api/subjects/${this.subjectId}/chapters/${this.chapterId}/quizzes/${quizId}`, {
                    method: "DELETE",
