export default {
  ...
  style="width: 100%; height: 200px; object-fit: contain; object-position: center;"
  alt="Subject Image"
/>


                <div class="card-body flex-grow-1">
                  <h5 class=" text-purple card-title">{{ subject.name }}</h5>
  ...
  methods: {
    ...
    async saveSubject() {
      ...
      const url = this.isEdit
        ? `${location.origin}/api/subjects/${this.subject.id}`
        : `${location.origin}/api/subjects`;
    ...
    async deleteSubject(subjectId) {
      ...
      const token = this.$store.state.auth_token;
      try {
        await fetch(`${location.origin}/api/subjects/${subjectId}`, {
          method: "DELETE",
